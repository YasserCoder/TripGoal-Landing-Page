...
bars.forEach((bar, i) => {
    bar.addEventListener("click", (e) => {
        ...
        if (bg.classList.contains("active")) {
            ...
            bg.classList.replace("desactive", "active") ||
    ...
    const lazyLoadBackground = (entries, observer) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
                ...
                const bgImage = backgroundElement.getAttribute("data-bg");
                backgroundElement.style.backgroundImage = bgImage;
                observer.unobserve(backgroundElement); // Stop observing after loading
            }
        });
    };
